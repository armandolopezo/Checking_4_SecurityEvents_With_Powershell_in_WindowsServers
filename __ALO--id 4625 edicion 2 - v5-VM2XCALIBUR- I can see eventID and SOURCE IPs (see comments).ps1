...
For ($x = 0; $x -lt $ArrayMaximumSize; $x = $x + 1)
   {
    ...
    $NewArrayList[$x] = [PSCustomObject]@{
     ...
     # I SourceAddress was wrong and I replaced ReplacementStrings[12] by ReplacementStrings[19] (7-7-2025) 
     ...
     if ($TestAddr -eq ("::1"))   {  }
     elseif ($TestAddr -eq ("-")) {  }
     # I added the following two lines in order to OMIT IP addresses 127.0.0.1 and 192.168.0.255 (7-7-2025)
     elseif ($TestAddr -eq ("127.0.0.1")) {  }
     elseif ($TestAddr -eq ("192.168.0.255")) {  }
     else
      {
