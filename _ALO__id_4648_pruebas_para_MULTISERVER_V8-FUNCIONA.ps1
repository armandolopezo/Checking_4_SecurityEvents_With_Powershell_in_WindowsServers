<#
.SYNOPSIS
    Monitoreo de eventos 4648 servidor por servidor con salida a archivo.
.DESCRIPTION
...
$PathReporte = "$env:USERPROFILE\Desktop\Reporte_4648_$Timestamp.txt"
...
$MonitorBlock = {
    try {
        ...
        foreach ($element in $result) {
            # En el evento 4648, el índice 12 suele ser la Source Network Address
...
$ContenidoArchivo = @()
$EncabezadoGeneral = "REPORTE CONSOLIDADO DE EVENTOS 4648 - FECHA: $(Get-Date)"
...
foreach ($srv in $Servidores) {
    ...
    try {
        ...
        if ($null -ne $DatosServidor) {
            ...
            $MsgWarn = "No se encontraron eventos 4648 en $srv (o la IP era local/vacía)."
