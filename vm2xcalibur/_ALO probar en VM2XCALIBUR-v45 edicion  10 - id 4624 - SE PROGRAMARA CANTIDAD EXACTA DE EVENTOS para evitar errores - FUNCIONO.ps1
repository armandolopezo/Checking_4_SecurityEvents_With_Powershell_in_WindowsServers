...
$Contador_de_eventos = $result.count

write-host "La siguiente linea muestra el número de eventos a revisar"
$Contador_de_eventos 
write-host " "
$results2 = @()
$results3 = @()     
$results2 = 1..10000 | ForEach-Object { "-" } # the following line is for a counter for valid IP address for a TRUE valid in the IF $TestAddr in the FOR  loop below.$y = 0# in the following LOOP I will process 10.000 events that is the limit for the $x variable
    for ($x = 0; $x -lt $Contador_de_eventos; $x = $x + 1) {
        $event = $result[$x]
        # the following 5 commented lines help me test a TEMPORARY FAILURE that said CANNOT INDE X NULL ARRAY ,and I don't know WHY.
        
        # write-host 'el siguiemte muestra valor de variable $EVENT'
        # $event 
        #write-host 'el siguiemte muestra valor $event.ReplacementStrings[18] '
