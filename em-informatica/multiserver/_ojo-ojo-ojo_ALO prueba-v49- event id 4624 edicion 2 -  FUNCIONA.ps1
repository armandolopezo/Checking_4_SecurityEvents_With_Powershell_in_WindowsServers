...
$results2 = $result
$results3 = @()  
# The following line is Previous creation if the second array to check 10.000 events    
# $results2 = 1..10000 | ForEach-Object { "" } # the following line is for a counter for valid IP address for a TRUE valid in the IF $TestAddr in the FOR  loop below.$y = 0# in the following LOOP I will process A LOT OF EVENTS  that is the limit for the $x variable
