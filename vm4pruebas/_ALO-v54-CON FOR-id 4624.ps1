...
For ($x = 0; $x -lt $ArrayMaximumSize; $x = $x + 1)
   {$element = $result[$x]
    ...
    $FilteredArray[$z] = [PSCustomObject]@{
      ...
      write-host " la IP es " $eventfiltered2.SourceAddr
      write-host " *** las 6 lineas de CODIGO anteriores del PROGRAMA FUENTE me diran SI AVANCE ************ "
  ...
  $FilteredIPaddressObject
  # $FilteredIPaddress = $FilteredIPaddressObject.SourceAddr
  # $FilteredIPaddress 
