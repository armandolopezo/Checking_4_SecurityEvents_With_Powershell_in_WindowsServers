...
For ($x = 0; $x -lt $ArrayMaximumSize; $x = $x + 1)
   {$element = $result[$x]
    ...
    $FilteredArray += "-"
    write-host " ********************** proximas lineas aparece sub-arreglo REPLACEMENTSstrings ******************************************* "
    $eventfiltered = $result[$x]
    $eventfiltered.ReplacementStrings
    write-host " la IP es "$eventfiltered.ReplacementStrings[18]
    write-host " ********************** ESTOY AVANZANDO PARA TROUBLESHOOTING CON ELSE Y ARREGLOS ******************************************* "
