...
For ($x = 0; $x -lt $ArrayMaximumSize; $x = $x + 1)
   {
    ...
    $NewArrayList[$x] = [PSCustomObject]@{
     ...
     write-host "la siguiente linea nuestra un valor de la VARIABLE EVENTFILTERED con el valor de un evento filtrado usando un valor indexado del arreglo NewArrayList."
     $eventfiltered
     pause
     write-host " *************************************************************************************************************************** "
